...
static int rpmsg_dev_match(struct device *dev, struct device_driver *drv)
{
    ...
    pr_info("rpdev->devier_override = %s drv_name = %s\n", rpdev->driver_override, drv->name);//add
	if (rpdev->driver_override)
		return !strcmp(rpdev->driver_override, drv->name);

    pr_info("driver_override miss match\n");//add
...
int rpmsg_register_device(struct rpmsg_device *rpdev)
{
    ...
    pr_info("before dev_set_name\n");//add
	//dev_set_name(&rpdev->dev, "%s.%s.%d.%d", dev_name(dev->parent),
	dev_set_name(&rpdev->dev, "%s.%d.%d", rpdev->id.name, rpdev->src, rpdev->dst);

	rpdev->dev.bus = &rpmsg_bus;
    pr_info("before device_register\n");//add
    pr_info("%s.%d.%d\n", rpdev->id.name, rpdev->src, rpdev->dst);//add
